...
const AdminDashboard = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <header className="flex justify-between items-center mb-12 bg-white p-6 rounded-2xl shadow-sm border border-zinc-100">
        ...
        <div 
          ...
          className={`
            ...
            ${isDragActive 
              ...
              : 'border-zinc-200 bg-white hover:border-zinc-400 hover:bg-zinc-50 shadow-sm hover:shadow-md'
            }
          `}
        >
          <input {...getInputProps()} />
          <div className="flex flex-col items-center gap-6">
            <div className={`p-6 rounded-full ${isDragActive ? 'bg-zinc-100' : 'bg-zinc-50'}`}>
              {uploadMutation.isPending ? (
                <Loader2 className="w-10 h-10 animate-spin text-zinc-600" />
              ) : (
                <Upload className={`w-10 h-10 ${isDragActive ? 'text-zinc-600' : 'text-zinc-400'}`} />
              )}
            </div>
            <div>
              ...
              <p className="text-zinc-500 mt-2">or click to select files</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {images?.map((img) => (
            <div key={img.key} className="relative group aspect-square bg-white rounded-2xl overflow-hidden border border-zinc-200 shadow-sm hover:shadow-lg transition-all">
              <img 
                src={img.url} 
                alt={img.key} 
